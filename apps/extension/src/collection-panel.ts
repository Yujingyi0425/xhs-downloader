...
export function createCollectionPanel(
  ...
  let controller: CollectionCaptureController | null = null;
  const close = (): void => { panel?.remove(); panel = null; controller?.cancel(); controller = null; };
  const toggle = (): void => {
    ...
    panel.querySelector("[data-start]")?.addEventListener("click", () => {
      ...
      controller = controllerFactory((count, round) => {
        status.textContent = "正在扫描";
        progress.textContent = `已发现 ${count} 条，第 ${round} 轮`;
      });
      void controller.start().then((result) => renderResult(status, progress, start, result)).finally(() => { controller = null; });
    });
  };
